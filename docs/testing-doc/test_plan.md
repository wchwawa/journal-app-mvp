...
> ⚠️ **Manual coverage**: The Realtime WebRTC session (`useVoiceAgent` ↔ `RealtimeSession`) requires a real browser plus an OpenAI Realtime environment, so it can currently only be exercised manually (floating button → connect → push‑to‑talk) with results recorded in the QA checklist. Automated tests only cover the state machine, timers, and supporting tool APIs. The team has run 100+ combinations of connections and function calls in a real environment, with conclusions captured in the QA checklist.
